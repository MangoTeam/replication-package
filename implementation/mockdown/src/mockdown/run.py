...
def run(input_data: MockdownInput, options: MockdownOptions, result_queue: Optional[Queue] = None) \
    ...
    if options.get('learning_method', 'noisetolerant') == 'heuristic': 
        learning_factory = NoiseTolerantLearning
        use_scores = False
    ...
    prune = pruner_factory(examples, bounds_dict, unambig, use_scores)
