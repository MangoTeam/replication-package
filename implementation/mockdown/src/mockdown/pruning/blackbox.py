...
class BlackBoxPruner(BasePruningMethod, Generic[NT]):
    ...
    def __init__(self, examples: Sequence[IView[NT]], bounds: ISizeBounds, solve_unambig: bool, use_scores: bool, targets: Optional[Sequence[IView[NT]]] = None):
...
class HierarchicalPruner(BasePruningMethod):
    ...
    def __init__(self, examples: List[IView[NT]], bounds: ISizeBounds, solve_unambig: bool, use_scores: bool):
    ...
    def integrate_constraints(self, examples: List[IView[NT]], min_c: Conformance, max_c: Conformance, constraints: List[IConstraint]) -> List[IConstraint]:
        result = BlackBoxPruner(examples, confs_to_bounds(min_c, max_c), self.solve_unambig, self.use_scores)(constraints)[0]
    ...
    def validate_output_constrs(self, constraints: Set[ConstraintCandidate]) -> None:
        solver = z3.Optimize()
        bb_solver = BlackBoxPruner([self.hierarchy], confs_to_bounds(self.min_conf, self.max_conf), self.solve_unambig, self.use_scores)
    ...
    def __call__(self, cands: List[ConstraintCandidate]) -> Tuple[List[IConstraint], Dict[str, Fraction], Dict[str, Fraction]]:
        ...
        while len(worklist) > 0:
            ...
            bb_solver = BlackBoxPruner(focus_examples, bounds, self.solve_unambig, self.use_scores, targets=targets)
